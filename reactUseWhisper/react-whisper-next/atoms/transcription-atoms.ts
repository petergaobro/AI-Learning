...
export const tokenSizeMessageAtom = atom((get) => {
  ...
  if (tokenSize.isTokenSizeExceeded) {
    ...
    return {
      type: "warning",
      message: `You are in the limit of the token size${tokenSize.maxTokenSize
        } however when you include the response (multiplying current token size (${tokenSize.currentTokenSize
        }) by 2 = ${tokenSize.currentTokenSize * 2
